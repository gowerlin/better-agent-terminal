...
### Follow-up：priority 顯示 Unknown（使用者要求一併修）

**追加修復**（commit `4d9fba4`）：
- 新增 `extractPriorityFromPlanContent(content)` 從 PLAN 檔案 metadata block 讀取「- **優先級**：」行
- `ControlTowerPanel.loadBacklog` 對 `priority === 'Unknown'` 的 entry，並行 fetch PLAN 檔案補真實 priority
- 結果：PLAN-012 顯示 `🔴 High`（非 `Unknown`）
- Build 通過，無 regression（只對原本 Unknown 的 entry 觸發補讀）
...
### Commit
- `cb0d535` — fix: recognize "Completed" section heading in CT panel backlog parser (BUG-036)
- `feb84df` — chore(ct): T0151 FIXED 元資料 + commit SHA
- `4d9fba4` — fix: enrich backlog entries with PLAN file priority when 優先級 column missing (BUG-036 follow-up)

### 互動紀錄
- [17:30] Q: 使用者回報「優先級：🔴 High BAT 清單還是 Unknown」 → A: 確認需一併修 priority → Action: 追加 `extractPriorityFromPlanContent` + `loadBacklog` enrich，commit `4d9fba4`
