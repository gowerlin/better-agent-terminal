...
- **T0181**（IN_PROGRESS，研究型）：PLAN-018 拆分研究，產出 T0182/T0183/T0184 完整規格（研究報告 `_report-t0181-plan-018-breakdown.md`）
- **T0182** TODO：TLS + fingerprint pinning + safeStorage + remoteFingerprint（預估 **5.5h**，T0164 原估 2-3h 已上修）
- **T0183** TODO：Path sandbox + image 10MB cap（預估 **1.9h**）
- **T0184** TODO：Brute-force 防護 + 連線穩定性（reconnect 退避 / mutex / maxPayload）（預估 **1.9h**）

**依賴關係**：T0182 → T0184（序列）；T0183 可與 T0182 並行。總工時 7-9h。

**實作完成判準**：三張全部 DONE + 整合測試場景 1-3 通過 + `version.json.lastSyncCommit = 5d9f486`。
