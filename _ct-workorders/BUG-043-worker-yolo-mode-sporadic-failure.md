...
## 元資料
- **編號**:BUG-043
- **狀態**:🚫 CLOSED(複測通過,使用者驗收)
- **嚴重度**:🟡 Medium(不影響工單功能,僅降級到剪貼簿 fallback,但破壞 yolo 閉環)
- **建立時間**:2026-04-19 00:00 (UTC+8)
- **關閉時間**:2026-04-19 01:25 (UTC+8)
- **關閉依據**:使用者複測通過。本 session T0194/T0195/T0196 三張手動派發期間 banner 缺失疑因 BUG-046 dispatcher 阻擋 + 手動派發不注入 CT_MODE 所致,非 BUG-043 本體;使用者另行測試正常派發鏈路後 banner 正常顯示 → 判定 BUG-043「偶發」描述可能為 BUG-046 副作用誤判。若 BUG-046 修復後仍再現則另開新 BUG 追蹤。
