...
- 使用者補充確認：Codex route 本應使用 `$ct-exec`，但 Tower skill BAT auto-session 規格硬編碼 `--agent default --prompt "/ct-exec T####"`。BAT UI ControlTowerPanel path 會依 `codex-cli` 產生 `codex "$ct-exec T####"`，因此錯前綴不是 BAT UI 問題，而是 Tower skill / BAT auto-session 派發協定問題。
- 推薦修法拆兩層：先在 Worker PTY 注入 `customEnv.MSYS_NO_PATHCONV='1'` 解 MSYS path rewrite；再修 Tower/BAT auto-session 協定，避免 `--agent default` 搭配固定 `/ct-exec`，Codex route 必須產生 `$ct-exec` 或由 BAT resolved agent 後 normalize prompt prefix。
