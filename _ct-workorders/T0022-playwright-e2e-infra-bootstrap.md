...
- **派發者**：Control Tower
- **前置工單**：T0021（commit BUG-006 hotfix，必須先 merge）
- **目標子專案**：（單一專案）
...
2. ✅ `main` 分支 working tree clean
3. ✅ `node_modules/` 可用（若懷疑過期，先跑 `npm install`）
...
7. **產生 2 個 atomic commits**
   ...
   - **Commit 2**：`chore(tower): dispatch T0022 playwright e2e infra`
...
7. ✅ 兩個 atomic commits（`feat(e2e)` + `chore(tower)`）
...
1. **環境檢查**
   - `cd` 到專案根目錄
   - `git status` 確認 working tree clean（T0021 已完成）
...
### Playwright Output
```
<npm run test:e2e 的 stdout 摘要>
```

### BAT 整合觀察報告
**在 BAT 終端執行情況**：
- [ ] `npm install` 執行：<正常 / 異常描述>
- [ ] Playwright browsers 下載：<正常 / 異常描述>
- [ ] `npm run test:e2e` 執行：<正常 / 異常描述>
- [ ] Electron app 啟動：<正常 / 異常描述>
- [ ] Playwright 控制 stdout/stderr：<正常 / 異常描述>
- [ ] ANSI color / cursor / 進度條：<正常 / 異常描述>
...
### 互動紀錄
<若有問塔台的問題或需要決策的 escalation>

### 遭遇問題
<若有 npm 安裝問題、版本衝突、TS 配置問題等>

### 學習鉤子候選
<若本次 infra bootstrap 揭露值得記錄的陷阱>
