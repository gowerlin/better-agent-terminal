# Backlog

> ⚠️ 此文件由 `*sync` 自動生成，請勿手動編輯。
> 最後更新：2026-04-19 00:55 (UTC+8)
>
> 統計：💡 Ideas: 4 | 📋 Planned: 3 | 🔄 In Progress: 0 | ✅ Done: 7 | 🚫 Dropped: 1
>
> 本 session 閉環：PLAN-019 DONE（TypeScript 技術債清理，tsc 133→2）
>
> 📦 已歸檔：4 張（PLAN-006 DROPPED, PLAN-008 DONE, PLAN-010 DONE, PLAN-011 DONE → _archive/plans/）
>
> Completed 熱區保留：PLAN-003 / PLAN-005 / PLAN-012 / PLAN-016 / PLAN-018 / PLAN-019 / PLAN-020（BUG-045 archive 面未閉環，暫不歸檔）

## Active

| ID | 標題 | 優先級 | 狀態 | 連結 |
|----|------|--------|------|------|
| PLAN-004 | GPU Whisper 加速（Win/Linux） | 🟡 Medium | 📋 PLANNED | [詳細](PLAN-004-gpu-whisper-acceleration-win-linux.md) |
| PLAN-009 | Sprint 儀表板 UI | 🟡 Medium | 📋 PLANNED | [詳細](PLAN-009-sprint-dashboard-ui.md) |
| PLAN-014 | BAT 內建 Git 圖形介面（方向 B） | 🟡 Medium | 📋 PLANNED | [詳細](PLAN-014-evaluate-vscode-extension-vs-git-gui.md) |
| PLAN-002 | Dynamic Import 衝突修復 | 🟢 Low | 💡 IDEA | [詳細](PLAN-002-dynamic-import-conflict-fix.md) |
| PLAN-007 | 遠端容器開發支援 | 🟢 Low | 💡 IDEA | [詳細](PLAN-007-remote-container-dev-support.md) |
| PLAN-013 | NSIS Installer 偵測檔案鎖定時詢問 kill Terminal Server | 🟢 Low | 💡 IDEA | [詳細](PLAN-013-installer-force-kill-on-file-lock.md) |
| PLAN-015 | Refactor：抽 renderPanelContent shared helper 消除雙 render 路徑 | 🟢 Low | 💡 IDEA | [詳細](PLAN-015-refactor-dual-render-path-shared-helper.md) |

## Completed

| ID | 標題 | 完成時間 | 連結 |
|----|------|---------|------|
| PLAN-019 | 清理 fork 既有 TypeScript 技術債（tsc 133→2，9 工單 + 7 commits） | 2026-04-18 23:30 | [詳細](PLAN-019-typescript-debt-cleanup.md) |
| PLAN-018 | Remote 連線資安加固（TLS + fingerprint + sandbox + anti-bruteforce） | 2026-04-18 21:00 | [詳細](PLAN-018-remote-security-hardening.md) |
| PLAN-020 | YOLO autonomous mode（Worker 無狀態化 v4.3.0） | 2026-04-18 16:05 | [詳細](PLAN-020-yolo-autonomous-mode.md) |
| PLAN-016 | Electron runtime 28.3.3 → 41.2.1 升級（三 Phase 全閉環） | 2026-04-18 05:30 | [詳細](PLAN-016-electron-runtime-upgrade-28-to-41.md) |
| PLAN-003 | npm audit 殘餘漏洞（Group A + B + C 三組全結案） | 2026-04-18 05:25 | [詳細](PLAN-003-npm-audit-remaining-vulnerabilities.md) |
| PLAN-005 | Electron Builder 升級（24.13.3 → 26.8.1） | 2026-04-18 05:25 | [詳細](PLAN-005-electron-builder-upgrade.md) |
| PLAN-012 | Quit Dialog 新增「一併結束 Terminal Server」CheckBox | 2026-04-17 17:12 | [詳細](PLAN-012-quit-dialog-terminal-server-checkbox.md) |

## Dropped

| ~~ID~~ | ~~標題~~ | 原因 | 連結 |
|--------|---------|------|------|
| ~~PLAN-001~~ | ~~Vite v5→v6 升級~~ | 被 PLAN-003 Group B 吸收（直接跨 major 升至 vite 8） | [PLAN-001](PLAN-001-vite-v5-to-v6-upgrade.md) |

> 已歸檔至 _archive/plans/：PLAN-006 / PLAN-008 / PLAN-010 / PLAN-011
