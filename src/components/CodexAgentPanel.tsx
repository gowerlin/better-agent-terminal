...
export function CodexAgentPanel({ sessionId, cwd, isActive, workspaceId, onClose, showUserMsg = true, showAssistantMsg = true, showToolMsg = true, showThinkingMsg = true }: Readonly<CodexAgentPanelProps>) {
  ...
  const handleSend = useCallback(async () => {
    ...
    if (!isStreaming && trimmed === '/resume') {
      ...
      try {
        const sessions = await window.electronAPI.claude.listSessions(cwd, 'codex-agent')
          ...
          sessionId: () => isCodexSession ? (
            ...
            <span key="sessionId" className="claude-statusline-item claude-statusline-clickable"
              onClick={async () => {
                ...
                try { setResumeSessions(await window.electronAPI.claude.listSessions(cwd, 'codex-agent') || []) }
